#!/bin/bash
# End-to-end admin-node onboarding: arm the PDM payload, discover the machine's
# MAC from its first (harmless) netboot attempt, then generate + serve its
# answer file. This is the Proxmox Datacenter Manager sibling of
# onboard-node.sh - PDM is the management product, so there is normally exactly
# ONE of these, not a fleet.
#
#     ~/scripts/onboard-admin.sh <name> [extra new-node.py flags...]
...
#
# NOTE: while pdm-fleet is armed it is the ONLY payload served. A PVE node that
# netboots during this window would get the PDM installer paired with its own
# PVE answer file. Run ~/scripts/pxe-stop.sh as soon as the admin node is done.
set -euo pipefail
...
if [ $# -eq 0 ]; then
    ...
    echo "  e.g. $(basename "$0") kay --ip 192.168.1.236" >&2
    exit 1
fi
NAME="$1"; shift

echo "==> arming PXE (pdm-fleet - Proxmox Datacenter Manager)"
sudo -A -A pxectl pdm-fleet
echo
...
echo
cd "$ONBOARD_DIR"
sudo ./new-node.py "$NAME" --mac "$MAC" --product pdm "$@" --serve

cat <<MSG

Done. Boot '$NAME' via PXE IPv4 again - it will now match and WIPE its disk
installing Proxmox Datacenter Manager unattended.
  ...
  turn PXE off : ~/scripts/pxe-stop.sh   <-- do this as soon as it is installed

The web UI is on port 8443 (PVE nodes are on 8006).
MSG
