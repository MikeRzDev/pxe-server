...
#
# Identical in every other way to onboard-node.sh - same payload machinery,
# same answer schema, same MAC-keyed answer server, which is shared between the
# products without conflict. Only three things differ: the payload armed
# (pdm-fleet, not proxmox-fleet), the --product passed to new-node.py, and the
# web UI port (8443, where PVE is on 8006).
#
# Give it --mac and a single power-on installs the machine unattended. With no
# --mac it falls back to two-pass discovery, which needs a SECOND MANUAL
# REBOOT. The target must boot via a UEFI **"...PXE IPv4..."** entry, not
# "...HTTP IPv4..." - see onboard-lib.sh for why, for the modes, for the
# wildcard narrowing, and for --keep-armed.
#
